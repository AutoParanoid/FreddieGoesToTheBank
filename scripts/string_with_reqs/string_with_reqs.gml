...
/// @arg level_reqs
/// @arg player_lvl
function string_with_reqs(argument0, argument1, argument2) {
	...
	for (var i = 0; i < len; ++i)
	{
		if (lvl_reqs[i] > player_lvl)
		{
			...
			for (var j = 1; j <= len_str; ++j)
			{
				...
				if ((string_ord_at(str_ar[i], j) >= 65 && string_ord_at(str_ar[i], j) <= 90) || (string_ord_at(str_ar[i], j) >= 97 && string_ord_at(str_ar[i], j) <= 122))
				{
					//str_ar[i] = string_set_byte_at(str_ar[i], j, ((string_ord_at(str_ar[i], j) + rand) % 49) + 65);
					str_ar[i] = string_set_byte_at(str_ar[i], j, 95);
					rand = randomise();
				}
			}	
		}
